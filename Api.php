...
class Api
{
    ...
    {
        return $this->sandbox ? 'https://sandbox.safaricom.co.ke' : 'https://api.safaricom.co.ke';
    }
    ...
    public function generateToken()
    {
        ...
        $url = $this->getApiEndpoint() . '/oauth/v1/generate?grant_type=client_credentials';
    ...
    public function reversal($Initiator, $SecurityCredential, $TransactionID, $Amount, $ReceiverParty, $RecieverIdentifierType, $ResultURL, $QueueTimeOutURL, $Remarks, $Occasion)
    {
        ...
        $url = $this->getApiEndpoint() . '/mpesa/reversal/v1/request';
    ...
    {
        //Command IDs SalaryPayment, BusinessPayment, PromotionPayment
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/b2c/v1/paymentrequest';
     ...
     * @return mixed
     */
    public function registerUrl($ValidationUrl, $ConfirmationUrl, $ShortCode)
    {
        $token = self::generateToken();

        $url = $this->getApiEndpoint() . '/mpesa/c2b/v1/registerurl';
     ...
     * @return mixed|string
     */
    public function c2b($ShortCode, $Amount, $Msisdn, $BillRefNumber)
    {
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/c2b/v1/simulate';
    ...
    {
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/accountbalance/v1/query';
    ...
    {
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/transactionstatus/v1/query';
    ...
    {
        // Command IDs:  BusinessPayBill, MerchantToMerchantTransfer, MerchantTransferFromMerchantToWorking, MerchantServicesMMFAccountTransfer, AgencyFloatAdvance
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/b2b/v1/paymentrequest';
     ...
     * @return mixed|string
     */
    public function STKPushSimulation($BusinessShortCode, $LipaNaMpesaPasskey, $TransactionType, $Amount, $PartyA, $PartyB, $PhoneNumber, $CallBackURL, $AccountReference)
    {
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/stkpush/v1/processrequest';
    ...
    {
        $token = self::generateToken();
        $url = $this->getApiEndpoint() . '/mpesa/stkpushquery/v1/query';
